...
\item For any modality $\modal$, the subuniverse of $\modal$-separated types consist of types with $\modal$-modal identity types. In other words, a type is $\modal$-separated if its diagonal is classified by $\UU_\modal$. The subuniverse of $\modal$-separated types is again a reflective subuniverse, and its reflection $\modal'$ is again a modality. Since all propositions are $\modal$-separated (because the unit type is $\modal$-modal), it follows that the $\modal'$-connected maps are surjective. Moreover, the following are equivalent for any map $f:A\to B$:
\begin{enumerate}
\item $f$ is $\modal'$-connected.
\item $f$ is surjective and its diagonal is $\modal$-connected.
\end{enumerate}
For any map $f:A\to B$ the following are also equivalent:
\begin{enumerate}
\item $f$ is $\modal'$-modal.
\item its diagonal is $\modal$-modal.
\end{enumerate}
\end{enumerate}
